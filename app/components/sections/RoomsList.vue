            ...
            <NuxtLink :to="localePath(`/rooms/${room.id}`)" class="group block relative">
              <article class="flex flex-col">
                ...
                <div class="mt-8 text-center space-y-3">
                  <span class="text-[10px] text-hotel-gold font-black tracking-[0.4em] uppercase block opacity-80">
                    {{ room.no }} — {{ room.cat }}
                  </span>
                  ...
                  <div class="flex items-center justify-center gap-4 pt-2">
                    <div v-for="feat in room.features?.slice(0, 3)" :key="feat.label" class="flex items-center gap-2">
                      ...
                      <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest">{{ t(`rooms.features.${feat.label}`) }}</span>
                    </div>
                  </div>
                </div>
